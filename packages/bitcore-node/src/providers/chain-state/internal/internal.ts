...
import {DiveAddressStorage} from '../../../models/diveAddress';
import {DiveOrganizationStorage} from '../../../models/diveOrganization';
...
export class InternalStateProvider implements IChainStateService {
  ...
  async getBalanceForAddress(params: GetBalanceForAddressParams) {
    ...
    if(!balance){
      balance = { confirmed: 0, unconfirmed: 0, balance: 0 };
    }
    let diveAddressOpt = await DiveAddressStorage.getDiveAddressOpt({address, network, chain});
    if(diveAddressOpt){
      let organization = await DiveOrganizationStorage.getDiveOrganization({org_id: diveAddressOpt.org_id, network, chain});
      balance.organization_name = organization.name;
    }
    return balance;
  }
